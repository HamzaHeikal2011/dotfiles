...
2. **Consolidation**: Sourced fragments are combined into single, cohesive `.conf` files in `~/.dotfiles/.claude/migration-stage/hypr/consolidated/`.
3. **Conversion**: The consolidated files are converted into `.lua` scripts in `~/.dotfiles/.claude/migration-stage/hypr/` using the `hl` helper.
...
| :--- | :--- | :--- | :--- |
| Main Entry | `hyprland.conf` | `hyprland.lua` | Verified |
...
1. **Sourced Changes**: If changes are made to files in `~/.dotfiles/sources/hypr/`, you MUST update the corresponding `.lua` file in `~/.dotfiles/.claude/migration-stage/hypr/`.
...
- **2026-06-16**: Created `hyprland.lua` as main entry point.
