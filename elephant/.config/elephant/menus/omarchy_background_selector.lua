...
function GetEntries()
	...
	local theme_name_file = io.open(home .. "~/.dotfiles/theme/theme.name", "r")
	...
	local dirs = {
		home .. "/.dotfiles/theme/backgrounds",
	}
	if theme_name then
		table.insert(dirs, home .. "~/.dotfiles/theme/backgrounds/" .. theme_name)
	end
			...
			for background in handle:lines() do
				...
				if filename and not seen[filename] then
					...
					table.insert(entries, {
						...
						Value = background,
						Actions = {
							activate = "~/.dotfiles/bin/theme-bg-set" .. ShellEscape(background),
						},
						Preview = background,
						PreviewType = "file",
					})
